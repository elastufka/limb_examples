...
;gets the angles between Mercury and Earth for the flares in Dennis's messenger list
    ...
    for i=0,len[0]-1 do begin
        flare_list.angle[i]=messenger_flare_angle([-200.,200.], strmid(flare_list.Datetimes.RHESSI_datetimes[i],1,12)) ;1,12
    endfor
...
;EM2
    ...
    for i=0,len[0]-1 do begin
        ...
        goes_fluxes,TMKelvin[i],flare_list.flare_properties.Messenger_EM1[i],flong,fshort,sat=15
        goesnum[i]=flong
        flare_list.flare_properties.Messenger_GOES[i]=goes_value2class(flong)
    endfor
    print, TMKelvin, flare_list.flare_properties.Messenger_EM1
    if plot ne 0 then begin
